...
export async function GET({ request }: { request: Request }) {
  ...
  const { data: settingRow } = await supabaseAdmin.from("app_settings").select("value").eq("key", "checkin_notifications").maybeSingle();
  const settings = (settingRow?.value ?? {}) as { enabled?: boolean; time?: string; timezone?: string; message?: string };
  ...
  if (now.hour + ":" + now.minute !== time) return Response.json({ ok: true, sent: 0, skipped: "not_due", time, timezone });
