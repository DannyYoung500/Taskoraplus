...
export const createAdvertiseCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const isAutomaticallyVerified = service.pricing_model === "watch_second" || service.platform === "telegram" || service.platform === "discord";
    const steps = service.pricing_model === "watch_second" ? ["Open the YouTube video in Taskora", `Watch for at least ${watchSeconds} seconds`, "Wait for automatic verification"] : ["Open the target link", `Complete: ${service.service_name}`, isAutomaticallyVerified ? "Wait for automatic verification" : "Submit a screenshot for review"];
    const { data: task, error: taskError } = await supabaseAdmin.from("tasks").insert({ platform:service.platform, title:data.title?.trim() || service.service_name, advertiser:"TASKORA Advertiser", reward:perTaskReward, slots_left:quantity, steps, proof:isAutomaticallyVerified ? "auto" : "screenshot", link:data.link.trim(), is_active:false, status:"draft", task_type:taskType, target:data.link.trim(), slots_total:quantity, budget, campaign_id:campaign.id, created_by:context.userId, instructions:service.pricing_model === "watch_second" ? `Verified watch duration: ${watchSeconds}s` : null } as never).select("*").single();
