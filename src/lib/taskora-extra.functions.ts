...
// RESTORE NOTICE: full file restored from last good commit; streak bonus applied below.
...
export const reviewWithdrawal = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabaseAdmin
      .from("withdrawals")
      .update({ status: nextStatus, notes: data.reason ?? null } as never)
      .eq("id", data.withdrawalId);
    if (error) throw new Error(error.message);
    if (data.decision === "rejected") {
      await supabaseAdmin.from("transactions").insert({
        user_id: row.user_id,
        label: `Withdrawal refund — ${row.method}`,
