...
export const ownerRegisterWebhook = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Prefer NO secret first — invalid TELEGRAM_WEBHOOK_SECRET is the #1 Bad Request cause
    ...
    if (!res?.ok) {
      ...
      res = (await fetch(
        `https://api.telegram.org/bot${token}/setWebhook?${q.toString()}`,
    ...
    if (!res?.ok) {
      ...
      const hint = /https|url|resolve|host|dns/i.test(desc)
        ? " Domain must resolve publicly (DNS). Prefer https://YOUR-PROJECT.vercel.app/api/telegram-webhook."
