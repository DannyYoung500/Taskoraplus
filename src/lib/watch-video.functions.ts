...
export const listWatchVideos = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    ...
    const { data, error } = await (supabaseAdmin as any)
      .from("watch_videos")
      .select("id,title,description,thumbnail_url,video_url,source_type,provider_name,provider_video_id,reward_usdt,reward_points,duration_seconds,status")
      .eq("status", "active")
...
export const startWatchVideo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: video } = await (supabaseAdmin as any)
      ...
      .eq("id", data.videoId).eq("status", "active").maybeSingle();
    ...
    const { data: existing } = await (supabaseAdmin as any)
      .from("watch_video_sessions").select("id,status").eq("video_id", data.videoId).eq("user_id", context.userId).eq("status", "started").maybeSingle();
    ...
    const { data: session, error } = await (supabaseAdmin as any)
      .from("watch_video_sessions").insert({ video_id: data.videoId, user_id: context.userId }).select("id").single();
...
export const completeWatchVideo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: session } = await (supabaseAdmin as any)
      .from("watch_video_sessions").select("id,video_id,user_id,started_at,status").eq("id", data.sessionId).eq("user_id", context.userId).maybeSingle();
    ...
    const { data: video } = await (supabaseAdmin as any)
      .from("watch_videos").select("id,source_type,duration_seconds,reward_usdt,reward_points,title,views_count").eq("id", session.video_id).maybeSingle();
    ...
    const { data: updated, error: updateError } = await (supabaseAdmin as any)
      ...
      .update({ status: "completed", completed_at: new Date().toISOString(), reward_usdt: Number(video.reward_usdt ?? 0), reward_points: Number(video.reward_points ?? 0) })
      .eq("id", session.id).eq("status", "started").select("id").maybeSingle();
    ...
    await (supabaseAdmin as any).from("watch_videos").update({ views_count: Number(video.views_count ?? 0) + 1, user_paid_usd: Number(video.user_paid_usd ?? 0) + rewardUsdt }).eq("id", video.id);
    return { ok: true as const, already: false as const, rewardUsdt, rewardPoints };
  });

export const createOwnerVideoUploadUrl = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
...
export const registerOwnerVideo = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { title: string; description?: string; videoPath: string; videoUrl: string; durationSeconds: number; rewardUsdt: number; rewardPoints: number }) => data)
  .handler(async ({ data, context }) => {
    await assertOwner(context.userId);
    ...
    const supabaseAdmin = await adminClient();
    const { data: row, error } = await (supabaseAdmin as any).from("watch_videos").insert({
      title: data.title.trim(), description: data.description?.trim() || null, video_url: data.videoUrl,
      source_type: "owner_uploaded", reward_usdt: data.rewardUsdt, reward_points: data.rewardPoints,
      duration_seconds: Math.max(0, Math.round(data.durationSeconds)), status: "active", created_by: context.userId,
    }).select("*").single();
    if (error) throw new Error(error.message);
